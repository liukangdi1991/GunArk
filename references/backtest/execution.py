...
def limit_up_price(prev_close: float, code: str = "", *, is_st: bool = False) -> float:
    return _round_tick(float(prev_close) * (1 + limit_pct_for_code(code, is_st=is_st)))
...
def is_limit_up(price: float, prev_close: float | None, code: str = "", eps: float = 1e-6, *, is_st: bool = False) -> bool:
    if prev_close is None or float(prev_close) <= 0:
        return False
    return float(price) >= limit_up_price(float(prev_close), code, is_st=is_st) - eps


def is_limit_down(price: float, prev_close: float | None, code: str = "", eps: float = 1e-6, *, is_st: bool = False) -> bool:
    if prev_close is None or float(prev_close) <= 0:
        return False
    return float(price) <= limit_down_price(float(prev_close), code, is_st=is_st) + eps
