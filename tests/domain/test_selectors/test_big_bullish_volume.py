...
from .helpers import make_big_bullish_volume_defn, make_empty_df, make_ohlcv_df
...
def test_empty_data():
    defn = make_big_bullish_volume_defn()
    result = _run(defn, make_empty_df())
    assert result.selected_codes == []
    assert result.strategy_id == "big_bullish_volume"
