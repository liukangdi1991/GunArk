...
from .helpers import make_volume_spike_balance_defn, make_empty_df, make_ohlcv_df
...
def test_empty_data():
    defn = make_volume_spike_balance_defn()
    result = _run(defn, make_empty_df())
    assert result.selected_codes == []
    assert result.strategy_id == "volume_spike_balance"
