...
def _run_selection_for_dates(
    ...
    from select_stock import (
        load_data_table, load_strategies_from_config,
    ...
    if strategies_filter:
        missing = [name for name in strategies_filter if name not in strategies]
        if missing:
            console.print(
                f"[yellow]⚠️ 以下策略在配置中不存在，已跳过: {', '.join(missing)}[/yellow]"
            )
...
示例:
  ...
  python run.py --strategies B1战法  只运行指定策略
    ...
    from select_stock import load_default_strategy_aliases
    default_aliases = load_default_strategy_aliases(Path(args.config))
    ...
    if args.month:
        ...
        strategy_filter = args.strategies or default_aliases
        if not args.strategies:
            console.print(
                f"[dim]🎯 默认策略: {', '.join(default_aliases)}[/dim]"
            )
            console.print()

        total_elapsed = _run_selection_for_dates(
            all_trade_dates, args.data_dir, args.config, RESULTS_DIR, console, strategy_filter
        )
        ...
        total_elapsed = _run_selection_for_dates(
            [latest], args.data_dir, args.config, RESULTS_DIR, console, strategy_filter
        )
