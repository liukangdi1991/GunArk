...
**边界未实测：退市股的 `adj_factor` 可得性**（`adj_factor` 改硬失败后新增的风险）
...
| 缺因子的退市股只数 | 后果 |
|---|---|
| > 5% × 有效清单（约 5,551 只时约 278 只） | 每轮都撞整批熔断 ⇒ **全量基线永远建不起来** |
...
**上线前必须先实测**（只读：不写 `stock_meta`、不写 `bars`、不动账本）：
...
- **点"全量重建"之前先跑退市股因子实测**：L∪D 与 `adj_factor` 硬失败**都已经在代码里**，而退市股的 `adj_factor` 可得性从未实测（§3.7 末「边界未实测」）。若系统性缺失，全量重建要么每轮撞熔断建不起来，要么建起来但永久少这批股。一条命令、只读、约 1–2 分钟：`TUSHARE_TOKEN=xxx .venv/bin/python scripts/check_delisted_adj_factor.py`（退出码 0 = 可以放心重建）。
