...
trendradar/infrastructure/
    ...
    tushare/writer.py         # _atomic_write_parquet / flush_by_code / staging 目录换名
...
**为什么每天都刷（不可省）**：§3.8 的原子提交使"日历幽灵日"（官方日历说是交易日、接口实际返回 0 行，如临时休市）的代价从"多花 2 次调用"升级为"**整个 stage-2 永久 failed**"。每天 1 次调用把这类日子的存活时间压到 1 天，是原子性的必要保险。
...
**为何 `ledger_suspect` 不直接映射到 `FULL`**（列在第 5 行、即在 `force` 之后）：若自动转全量，则一次自检失败会让用户的下一次普通点击静默变成 44 分钟 / 11,778 次调用的重建，可能直接烧掉当日额度。`REBUILD_REQUIRED` 的含义是“**拒绝执行任何拉取**，面板 ⑤ 明告原因并要求用户显式点全量重建”（届时 `force` 为真即命中第 3 行走 `FULL`）。`sync_meta.ledger_suspect` 仅在**一次全量成功（含带缺口显式提交）**后清除。
...
| 全量（按股票） | 2 次/只 × 5,889 = 11,778 次 | **约 44 分钟** |
...
② for d in plan.missing_days（升序，串行）:
      ...
      断言① 未通过 → 该日记入 doubtful_days，继续下一天（见 §3.8 处置分类）
...
⑤ 读回校验：scan 全库实测日历（5,211 文件实测 1.3 秒）→ 断言②④
   （doubtful_days 已在 ② 剔除，不进入声称集合，故不会拖累整批）
⑥ commit.py：单事务写 sync_done_days（仅成功批）+ 更新 sync_meta
```
...
→ 自检（staging 副本）
→ 通过：整目录原子换名（bars ↔ bars_staging），旧目录改名保留为上一版
        + 单事务用本次覆盖区间替换 sync_done_days
→ 不通过 / 取消 / env 失败：staging 原地保留（下次续传），作业 failed，本地数据不变
```

真删本地再重拉被否决：11,778 次调用在限流下最短 44 分钟，中途失败会留下"回测选股全废且无法回退"的状态；换名方案 100% 保留"要么全新要么原样"的语义（本地数据实测 81 MB / 5,211 文件 / 单文件 16 KB，暂存代价可忽略），且失败自动回退。
...
- **通道一（自动）**：每次 stage-2 运行结尾，只要 `sync_skipped` 非空（**含 `UPTODATE` 与 `INCREMENTAL` 两种批**，否则无缺口时名单永无重试机会）即自动带一轮 `BACKFILL_CODES` 补齐，成本 N × 2 次调用（N 通常为 0），成功即销账；
- **通道二（手动）**：面板"立即补齐"按钮触发同一批，并先清空 `attempts` 计数（用于误判后重新给 3 次机会）。
...
| # | 断言 | 时机 | 抓的是哪类鬼 |
|---|---|---|---|
| ① | 每个声称完成的交易日：`行数 ≥ 0.75 × expected(d)`，其中 `expected(d) = |{s ∈ 清单 : s.list_date ≤ d 且 (s.delist_date 为空 或 s.delist_date ≥ d)}|` | 内存副本 | 接口返回空表 / 只回半截（限流受害者） |
...
比值区间 0.854~1.007：2015 年偏低是当年大面积停牌（真实缺口，非数据损坏），故取 0.75 留 14% 缓冲；而"只回半截"的比值约 0.5，仍能稳定报警。比值可略超 1（`stock_basic` 快照滞后），因此只做下限断言。

**失败处置分三档，不是一句“全失败”**（差别在于“失败说明了什么”）：

| 断言 | 失败说明 | 处置 | 置 `ledger_suspect` |
|---|---|---|---|
| ① 单日行数比值不足 | 只有**这一天**的接口响应可疑（休市/半截） | 该日不入账，**其余日照常写盘与落账**；作业 `failed`；面板红字“N 个交易日行数异常已跳过，请核对是否大面积停牌” | 否 |
| ③ 副本结构异常（重复日期 / NaN） | **合并逻辑本身**出错，可能污染多个文件 | 整批不落账 | 是 |
| ②④ 读回缺失 | 账本与文件已漂移 | 整批不落账 | 是 |
| 全量批任意断言失败 | 基线不可信 | 丢弃 staging（保留续传）、本地不变 | 是 |
...
| stage-2 行情 | `market_bars_sync` | `queued → running → success/failed/cancelled`（增量 0.5 秒~3 分钟；全量约 44 分钟，可跨天） |

`running` 必须保留（面板需区分"排队中"与"正在跑"）；`queued` 沿用现名，不引入 `pending`。一次 UI 点击 = stage-1 一行 + stage-2 一行（stage-1 由 stage-2 起跑前内部先行触发）。

需一并修掉的现状坑：

1. `executor.py:36-43` 的互斥判断硬编码 `job_type == "market_sync"` —— 改名后互斥静默失效，改为集合同判（`{"market_bars_sync"}`）；同一 stage 的"查表 + 插入"需在同一事务内完成以消除 TOCTOU。
...
⑤ ⚠ 账本曾被判不可信 / N 个交易日行数异常已跳过          ← ledger_suspect 或 marker_mismatch 或 doubtful_days 非空才出现
                                                        ← suspect 时额外附“需手动全量重建”提示与按钮
...
| R6 | 断言① | 造 2015 场景 `2335/2733 = 0.854` → 通过；造 `0.5` → **仅该日不入账**，同批其余日照常落账、作业 `failed`、`ledger_suspect` 保持 false（验证 §3.8 的例外不致永久堵死） |
...
| R10 | §3.6 | 全量中途取消/失败 → 断言 `bars/` 目录内容一字未改（换名前）；成功 → 断言换名后旧版本目录仍存在 |
| R11 | §3.9 | `presenters` 各态单测（stage-1 failed 阻断、`trusted_through=null`、`missing_codes>0`）+ `test_api_contract.py` 契约；executor 互斥覆盖 `market_bars_sync` 与终态不覆写 |
| R12 | §5 | `test_schema.py` 断言四新表存在、`market_sync_runs` 已移除；`stocklist` 断言 L ∪ D 合并与 `delist_date` 列 |
...
- **全量可合法跨天完成**：若撞上 token 当日额度上限，作业 `failed(env)` 且 staging 原地保留；次日再点一次即从断点续拉（`env` 不计次，不会误杀）。全量清单实测 5,889 只 = 11,778 次调用 ≈ 44 分钟地板。
