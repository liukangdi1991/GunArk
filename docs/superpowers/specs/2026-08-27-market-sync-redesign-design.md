...
| 类别 | 判据 | 计入该股失败次数 |
|---|---|---|
| `env` | 频率超限 / 每分钟上限 / bucket 取令牌超时 / 网络不可达 / HTTP 5xx / token 当日额度耗尽 / **按日**拉取时整天 `adj_factor` 空（`fetch_day_by_date` 一次取全市场 ⇒ 整天因子表还没发布，属时序问题，下轮自愈） | **否，一次都不计**（INV-5） |
| `code` | 接口明确拒绝该参数（"参数错误"/"无此股票"）、返回结构无法解析、**逐股**拉取时该股 `adj_factor` 空或覆盖不全（`fetch_code_range` 一次只取一只 ⇒ 就是这只股自己的事） | 是 |
| `unknown` | 其它异常 | 是，`last_error` 原文入库供人工判定 |
| `ok_empty` | 钳制后窗口内本就没有交易日。**「理论不该出现」是错的**——实测会稳定出现：退市股在正式退市前普遍长期停牌，最后交易日落在 `BASELINE_START` 之前（2026-09-02 实测 252 只退市股中 2 只：`000562` 最后交易日 2014-12-09、`601268` 2014-04-28，2015 全年 0 行） | 否，视为成功（不写文件、不计次、不出列）|
...
§1 只实测过退市股 `daily` 可拉（`000004.SZ` 2,561 行），**没实测退市股的 `adj_factor`**。
而 `adj_factor` 现在取不到即硬失败（`fetch.AdjFactorUnavailable`，不再静默写占位 1.0），
逐股路径判 `code` ⇒ 计次 ⇒ 3 轮出列。若 Tushare 对退市股系统性不给因子，「并入退市股
修正幸存者偏差」这件事本身就会变成一个永久缺口，且按只数分两种坏法：

| 缺因子的退市股只数 | 后果 |
|---|---|
| > 5% × 有效清单（实测分母 5,467 ⇒ 273 只） | 每轮都撞整批熔断 ⇒ **全量基线永远建不起来** |
| ≤ 5% | 健康轮 ⇒ 3 轮出列 ⇒ 带缺口提交 ⇒ **永久少这批股**（幸存者偏差换个形式回来） |

这也暴露了上面熔断那段「即使分类判错也不会误杀」的一个洞：熔断保护的是**不因误判而
把股票记出局**，但当失败源是**永久性数据边界**而非瞬时环境故障时，它不会误杀——它会
**死锁**，每轮都判环境故障、每轮都不计次、每轮都建不起来，且文案一直说「等下轮就好」。
所以这一档只能实测排除，推理不可信。

**实测结果**（真 token，逐股走生产同一条 `build_effective_list` + `fetch_code_range`）：

| 项 | 值 |
|---|---|
| `stock_basic` L∪D 归一化 | 5,893 只（其中退市股 339 只） |
| 有效清单 | 5,467 只，含退市股 **252** 只（339 − 82 只退市早于 `BASELINE_START` 被 ④ 钳掉，余下差值为北交所等过滤） |
| 整批熔断阈值 | 失败 > 273 只 |
| 对照组 10 只在市股 | 0 异常 ⇒ 接口本身通，退市组结论有效 |
| 退市组 252 只 | 250 只实拉到行 / 2 只 `outside_baseline` / **0 只缺因子**，退出码 0 |

三条结论：

1. **Tushare 对退市股照给 `adj_factor`** ⇒ 硬失败不会在退市股上制造缺口，L∪D 修正
   幸存者偏差不会被自己的保险堵死；
2. 上表第一行那一档**从退市股这条来源数学上不可达**：退市股总共 252 只 < 273 只，
   就算全缺也撞不到熔断，只会走「带缺口提交」那一档。熔断真正防的是限流/额度同时打挂
   一批在市股，对永久性边界本来就不起作用——上面那段「死锁」推导本身仍然成立，只是
   退市股不是触发它的场景；
3. 那 2 只 `outside_baseline`（`000562` 最后交易日 2014-12-09、`601268` 2014-04-28，
   2015 全年 0 行）**不是缺口**：退市前长期停牌，Tushare 有它们的数据、只是全落在基线
   之前，生产按 `ok_empty` 视为成功（不写文件、不计次、不出列）。它们同时证伪了分类表
   里 `ok_empty`「理论不该出现」的说法，那一行已按实测改写。

**上线前的前置检查**（只读：不写 `stock_meta`、不写 `bars`、不动账本）：

```bash
TUSHARE_TOKEN=xxx .venv/bin/python scripts/check_delisted_adj_factor.py
# 先抽样看苗头加 --limit 50；面板配了 exclude_boards 就带 --exclude-boards，否则分母不对
```
...
退出码 **0 = 每一只被测股的因子都被真的查过且拿到了**（2026-09-02 实跑即为此档）。
注意不能只看"有没有空因子响应"：`fetch_code_range` 在 daily 无行时返回 `kind=None` +
空帧（`OK_EMPTY` 是 `runner.py:124` 分片全空时才造的），而 `_attach_adj_factor` 在
`df.is_empty()` 处提前 return、**根本没查因子**。所以窗口无行的股要回看更早区间再分两档：
`outside_baseline`（更早区间有行 ⇒ 停牌在基线之前，良性，仍退 0）与 `daily_empty`
（任何区间都无行 ⇒ "无证据"而非"无风险"，退 1 拒绝下结论）。退 1 也可能是命中上表某一行。
有效清单或 Tushare 口径明显变化后需重跑——它是一条命令、只读、不计次不落盘的保险。

**三种候选兜底口径：一律不实现**。实测缺口为 0，写进去就是死重，还会削弱刚建立的硬失败
保证。留档是因为下面这份推导里两件东西与本次结论无关而仍然成立：缺因子时的代价链（`code`
计次 → 3 轮出列 → 带缺口提交），以及「部分 null 比整列 1.0 更隐蔽」这条下游事实。若日后
前置脚本报出缺口，按此三选一：

1. **存 `adj_factor = null` 而非出列**：行情行照存，因子列整列为 null。下游 `_qfq_scale`
   的既有守卫（`latest is None or latest <= 0 → return None`）会让该股被 selector 跳过
   ——数据在、指标诚实地不可算，洞从「这只股不存在」变成「这只股没有复权数据」。
   **前提是整列 null，绝不能部分 null**：`rolling_mean(min_samples=1)` 会跳过 null 静默
   缩短窗口，部分 null 比整列 1.0 更隐蔽；
2. **只对退市股豁免**：有 `delist_date` 的股容忍因子缺失（按 1 存 null），在市股仍硬失败
   ——在市股缺因子是真异常，退市股缺因子可能只是接口边界；
3. **维持现状（出列）**：接受这批股永久缺席，但面板必须把「因 `adj_factor` 缺失而出列」
   与其他出列原因区分显示，否则又变回看不见的洞。

若真要选，倾向 1 或 2；3 仅在缺失只数为个位数时可接受。
...
- **全量可合法跨天完成**：若撞上 token 当日额度上限，作业 `failed(env)` 且 staging 原地保留；次日再点一次即从断点续拉（`env` 不计次，不会误杀）。全量清单（L∪D 剔除北交所/退市早于基线者）实测 5,467 只 ≈ 10,934 次调用 ≈ 41 分钟地板。
...
- **退市股**：并入退市股（`stock_basic` D 档 339 只，其中 252 只落在基线内进有效清单）后回测样本含已退市标的；若某些策略假设"股票池恒为在市股票"，需要在看数时意识到这一点（这是修正幸存者偏差的预期结果）。
- **点"全量重建"之前先跑退市股因子实测**：L∪D 与 `adj_factor` 硬失败**都已经在代码里**。若退市股系统性缺因子，全量重建要么每轮撞熔断建不起来，要么建起来但永久少这批股（推导见 §3.7 末「边界实测」一节）。**2026-09-02 已跑过并通过**：252 只在有效清单内的退市股，因子无一缺失（退出码 0），风险不存在。有效清单显著变化后重跑一次即可，一条命令、只读：`TUSHARE_TOKEN=xxx .venv/bin/python scripts/check_delisted_adj_factor.py`（退出码 0 = 可以放心重建）。
