...
结论先行：2026-09-02 实跑通过（当时有效清单 5,467 只、退市股 252 只，0 只缺因子，
退出码 0）；2026-09-10 R24（BJ 入清单）后重跑：有效清单 **5,818 只、退市股 257 只、
熔断阈值 290 只**，仍 0 缺因子。风险不存在。脚本保留，因为「保留 L∪D + 硬失败」
这个组合的前提就是这个数；有效清单显著变化或 Tushare 口径变更时重跑一条命令即可，
不用重新推理一遍。
...
越过 5% × 有效清单**（R24 后实测分母 5,818 ⇒ 阈值 290 只；退市股总共才 257 只，第一档
数学上不可达）。本脚本直接把这个数算出来。
